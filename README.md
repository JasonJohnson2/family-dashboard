...
- **Automatic sync:** cron checks every 15 minutes; opening/returning to the dashboard and its visible one-minute timer request catch-up when Google data is at least 10 minutes old. A durable 10-minute attempt cooldown and the existing lease prevent Google request storms. Household loading stays independent of Google. Failed calendars keep cached events and expose sanitized status at `GET /api/google/refresh`; other calendars continue.
- **Operation:** connect, discover, and enable selected calendars using the documented protected API workflow; automatic sync keeps them current. Administrative `POST /api/google/sync` still forces a sync. No new Cloudflare secrets or manual cron setup are needed after pipeline deployment. No Google writes or webhooks. Phase 2 will add the protected connection-management UI.
