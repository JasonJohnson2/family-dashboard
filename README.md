...
- **Migrations:** `0002_google_calendar.sql`, `0003_google_calendar_members.sql`, and `0004_google_sync_status.sql`; use `pnpm db:migrate:local` locally. The deployment pipeline runs `pnpm db:migrate:remote` before publishing.
...
- **Calendar sync:** no background cron. Opening/returning to Calendar checks for Google data at least one hour old. Fresh/no-op checks perform no D1 writes. **Sync calendars** bypasses staleness with a short server-side cooldown and shows progress/errors. Household loading stays independent of Google; cached events survive failures.
- **D1 efficiency:** incremental and full refreshes write only changed event projections/member links and only bump household revision for visible changes. No-change sync writes a small constant amount of metadata, independent of the number of cached events.
- **Operation:** connect, discover, and enable selected calendars using the documented protected API workflow. Administrative `POST /api/google/sync` still forces a sync. The pipeline removes the old cron via `triggers.crons: []`; no manual Cloudflare change, new migration or new secrets are needed. No Google writes or webhooks.
